...
// Global tracking arrays for clean memory recycling
...
void hookCountries(Gateway& c1, Gateway& ci, u32 countryID) {
  ...
  static bool country1UpgradeSet = false;
  if (!country1UpgradeSet) {
    country1UpgradeSet = true;
    c1.onUpgrade([](Packet& pkt, Tunnel& tunnel, Cart& cart) -> RoutingEntry* {
  ...
  cli->onReady([&c1, &ci, cli, ciWire, countryID](Packet pkt, Cart cart) {
...
Gateway* getOrCreateGateway(const Resource::NumericalAddress& addr) {
  ...
  if (addr.size() == 1) {
    ...
    cli->onReady([parent, gw, cli, childWire, key](Packet pkt, Cart cart) {
...
int main() {
  ...
  for (int run = 1; run <= 1; ++run) {
    printf("\n--- Run %d/1 (20 Parallel Clients & Servers) ---\n", run);
    ...
    for (int pIdx = 0; pIdx < 20; ++pIdx) {
      ...
      Gateway* clientGW = getOrCreateGateway(cliPath.toAddress());
      Gateway* serverGW = getOrCreateGateway(srvPath.toAddress());
      ...
      srv->onPacket([pIdx](Packet pkt, Tunnel& tunnel, Cart cart) {
        printf("[DEBUG] Server %d received packet of size %zu, echoing back.\n", pIdx, pkt.payload.size());
        // Echo back the packet payload
        tunnel.push(pkt);
      });
      globalServers.push(srv);
      ...
      cli->onReady([cli, testPayload, pIdx](Packet pkt, Cart cart) {
        printf("[DEBUG] App Client %d ready! Pushing 64KB payload...\n", pIdx);
    ...
    while (!allDone && ticks < 2000) {
      ...
      for (usz i = 0; i < appClients.size(); ++i) {
        appClients[i]->update();
        if (appClients[i]->tunnel) {
          appClients[i]->pushCart();
        }
      }
      for (usz i = 0; i < linkClients.size(); ++i) {
        linkClients[i]->update();
        if (linkClients[i]->tunnel) {
          linkClients[i]->pushCart();
        }
      }

      // Update all active server processes
      for (usz i = 0; i < globalServers.size(); ++i) {
        globalServers[i]->update();
      }
      ...
      usleep(2000); // 2ms tick delay
    }

    if (allDone) {
      printf("  ✓ All 20 parallel pairs succeeded in %d ticks.\n", ticks);
    } else {
      printf("  ✗ Run timed out after %d ticks.\n", ticks);
      int failed = 0;
      for (usz i = 0; i < doneFlags.size(); ++i) {
        if (!(*doneFlags[i])) failed++;
      }
      printf("    Failed pairs: %d/20\n", failed);
      assert(false && "Timeout during heavy parallel execution!");
    }

    // Clean up memory before starting the next run
    cleanup();
  }
